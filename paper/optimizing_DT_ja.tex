...
\subsubsection{認知負荷と誤回答リスクの評価}

質問統合は質問数を削減する一方で，選択肢の増加により認知負荷と誤回答リスクを増加させる可能性がある．統合の妥当性を評価するため，以下の指標を定義する．

\begin{definition}[認知負荷指標]
質問 $q$ に対して以下の指標を定義する．
\begin{enumerate}
    \item \textbf{選択肢数} $N_{\text{opt}}(q) = |A(q)|$：回答選択肢の数
    \item \textbf{読み時間} $T_{\text{read}}(q) = \alpha \cdot L_{\text{text}}(q) + \beta \cdot N_{\text{opt}}(q)$：質問文と選択肢の読了時間（$L_{\text{text}}$ は文字数，$\alpha, \beta$ は係数）
    \item \textbf{認知複雑度} $C(q) = N_{\text{opt}}(q) \cdot \log_2(N_{\text{opt}}(q) + 1)$：選択肢間の比較負荷
\end{enumerate}
\end{definition}

\begin{definition}[誤回答リスク指標]
質問 $q$ の誤回答リスクを以下で評価する．
\begin{enumerate}
    \item \textbf{曖昧度} $\text{Amb}(q) = \frac{|\{(a_i, a_j) : \text{sim}(a_i, a_j) > \theta\}|}{N_{\text{opt}}(q)^2}$：意味的に類似した選択肢ペアの割合
    \item \textbf{誤り伝播リスク} $R_{\text{prop}}(q) = \sum_{a \in A(q)} p(a) \cdot d_{\text{sub}}(a)$：誤回答時の後続パスへの影響（$d_{\text{sub}}$ は部分木の深さ）
\end{enumerate}
\end{definition}

\begin{table}[h]
\centering
\caption{質問統合の認知負荷評価（EU AI Act チェッカーの例）}
\begin{tabular}{@{}lccccc@{}}
\toprule
\textbf{統合対象} & \textbf{元質問数} & \textbf{元選択肢計} & \textbf{統合後選択肢} & \textbf{$C$ 変化} & \textbf{リスク} \\
\midrule
QAIS 6.4.1--6.4.7 & 7 & 14 & 8 & $7 \times 1 \to 24$ & 中 \\
Q1 + QAIS 1 & 2 & 4 & 3 & $2 \times 1 \to 4.75$ & 低 \\
Annex I カテゴリ & 4 & 16 & 12 & $4 \times 4 \to 43$ & 高 \\
\bottomrule
\end{tabular}
\end{table}
...
\textbf{評価基準}：
\begin{itemize}
    \item $N_{\text{opt}} \leq 7$：Miller の法則に基づく認知限界内
    \item $C_{\text{after}} / C_{\text{before}} \leq 3$：認知負荷増加が許容範囲
    \item $\text{Amb}(q_m) < 0.1$：曖昧度が低い
\end{itemize}

QAIS 6.4.1--6.4.7 の統合は $N_{\text{opt}} = 8 \leq 7$ を僅かに超過するが，カテゴリが明確に区分されているため曖昧度は低く，統合が妥当と判断される．一方，Annex I の全カテゴリ統合は $N_{\text{opt}} = 12$ となり認知限界を超えるため推奨されない．

\subsubsection{情報保存定理}

質問統合において情報利得が保存される条件を明確化する．

\begin{theorem}[Information Preservation]
質問 $q_1, q_2, \ldots, q_k$ を統合した質問 $q_m$ について，以下の条件が成立するとき，
\begin{equation}
...
\textbf{条件：}
\begin{enumerate}
    \item \textbf{排他性}：$\forall i \neq j: A(q_i) \cap A(q_j) = \emptyset$（回答集合が互いに素）
    \item \textbf{網羅性}：$A(q_m) = \bigcup_{i=1}^{k} A(q_i)$（統合質問が全選択肢を含む）
    \item \textbf{分割整合性}：各 $a \in A(q_m)$ に対し，元質問での条件付き分布が保存される
    \item \textbf{独立性}：$q_i$ と $q_j$ の回答が統計的に独立（$i \neq j$）
\end{enumerate}
\end{theorem}

\begin{proof}[証明スケッチ]
$q_m$ の回答集合を $A(q_m) = \{a_1, \ldots, a_n\}$，状態空間を $\mathcal{S}$ とする．

情報利得の定義より：
\begin{equation}
\text{IG}(q_m) = H(\mathcal{S}) - \sum_{a \in A(q_m)} p(a) \cdot H(\mathcal{S} | q_m = a)
\end{equation}

条件1（排他性）と条件2（網羅性）より，$A(q_m)$ は $\mathcal{S}$ の分割を誘導する．元質問 $q_i$ は $A(q_i) \subset A(q_m)$ のみに基づく粗い分割を誘導するため，
\begin{equation}
H(\mathcal{S} | q_m) \leq H(\mathcal{S} | q_i) \quad \forall i
\end{equation}
が成立する（細かい分割ほど条件付きエントロピーが小さい）．

よって，
\begin{equation}
\text{IG}(q_m) = H(\mathcal{S}) - H(\mathcal{S} | q_m) \geq H(\mathcal{S}) - H(\mathcal{S} | q_i) = \text{IG}(q_i)
\end{equation}
が全ての $i$ について成立し，$\text{IG}(q_m) \geq \max_i \text{IG}(q_i)$ が示される．
\end{proof}

\textbf{反例と回避条件}：条件4（独立性）が崩れる場合，統合により情報が失われる可能性がある．例えば，$q_1$ の回答が $q_2$ の回答を制約する場合（条件付き依存），単純な統合では元の分岐構造を再現できない．この場合，動的な条件分岐を統合質問内に埋め込むか，統合を避ける必要がある．
