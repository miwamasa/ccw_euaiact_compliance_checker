...
**Optimization Engine:** `optimization_engine.py` (programmatic analysis)
...
| END Terminal States | 20 |
| Total Paths Enumerated | 7,762 |

### Path Length Statistics (Calculated)
...
| Avg Path Length (AI System) | 8.6 | 6-7 | **20-30% reduction** |
| Avg Path Length (GPAI) | 5-7 | 3-4 | **30-40% reduction** |
...
### 2. Early Termination Points (Calculated by Engine)

| Question | Terminal Probability | Avg Depth |
|----------|---------------------|-----------|
| QAIS 5 | **61.5%** | 7.7 |
| QAIS 7.1 | **100.0%** | 11.9 |
| QGPAI 3 | **38.9%** | 4.0 |
...
### 3. Top Information Gain Questions

| Question | Information Gain | Paths Through | Terminal Prob |
|----------|-----------------|---------------|---------------|
...
- 10 questions with similar routing complexity
- 21 questions with similar routing complexity (QAIS 1.x, 2.x, 6.x)

**Reordering Recommendations:**
- High IG questions appearing late: QAIS 4, QAIS 5, QAIS 6.1, QAIS 6.2, QAIS 6.3
- Consider moving earlier for better discrimination

**Low Value Questions (IG < 0.1):**
- Q1, QAIS 1 - consider merging or streamlining
...
| `original_checker_ec.yaml` | YAML format of original (with routing) |
| `optimized_checker_ec.yaml` | Optimized version |
| `eu_ai_act_checker_ja.html` | Japanese HTML app with EC routing |
| `optimization_engine.py` | **Programmatic optimization engine** |
| `optimization_report.json` | Full analysis report (generated) |
| `optimized_flow.json` | Recommended optimized flow (generated) |
| `convert_json_to_yaml.py` | JSON to YAML converter with routing |
